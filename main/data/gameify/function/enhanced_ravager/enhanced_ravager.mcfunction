...
execute as @s[scores={time=300..}] at @s run data merge entity @s {Motion:[0d,0.65d,0d]}
...
execute as @s[tag=Shockwave] at @s if entity @e[tag=RavagerTarget] unless block ~ ~-0.05 ~ #air as @e[distance=..20,type=!item_frame,type=!glow_item_frame,type=!painting,type=!pillager,type=!ravager] at @s unless block ~ ~-0.05 ~ air run damage @s 12 explosion by @e[tag=RavagerMarker,limit=1] from @e[tag=RavagerMarker,limit=1]
