...
execute as @s if block ~ ~ ~ spawner{components:{"minecraft:custom_data":{custom_crafting:1}}} align xyz positioned ~0.5 ~ ~0.5 unless entity @e[tag=CustomCrafter,distance=..0.5] run summon marker ~ ~ ~ {Tags:["CustomCrafter","CustomCrafterInitial"],NoGravity:true,Invisible:true}
# setblock ~ ~ ~ bedrock
